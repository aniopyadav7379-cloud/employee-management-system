...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (res && res.token) {
        // ✅ FIX: Save remember-me BEFORE redirect
        ...
        // ✅ FIX: res could be null (network issue already caught) or missing token
        showAlert('Login failed. Please check your credentials and try again.');
      }

    } catch (err) {
      // ✅ FIX: Friendlier messages for common backend errors
      const msg = err.message || '';
      if (msg.toLowerCase().includes('user not found') || msg.includes('401')) {
        ...
        showAlert('Cannot reach the server. It may be starting up — please wait 30 seconds and try again.');
      } else {
        showAlert(msg || 'Login failed. Please check your credentials.');
      }
    } finally {
      setLoading(false);
    }
  ...
  if (savedEmail && emailIn) {
    emailIn.value = savedEmail;
    if (rememberMe) rememberMe.checked = true;
  }

  /* ── Demo Credential Quick-fill ───────────────────────────── */
  // Click any <code> inside .demo-creds to auto-fill email or password
